...
#loading Data
raw = pd.read_csv("resources/train.csv.csv")
clean = pd.read_csv("resources/Images/clean")
img3 = Image.open("resources/Images/banksy.jpg")


#Main function to run the app
def main():
	"""TeanJM Tweet Classifier Applications"""
	
    
	activities = ["App Overview","About Global Warming","Exploratory Data Analysis","Prediction","NLP","Machine Learning Models"]  
	selection = st.sidebar.selectbox("Choose Activity",activities)
	...
	if selection == "App Overview" :
			...
			img2 = Image.open("resources\Images\gw.jpeg.jpg")
	...
	if selection == "Machine Learning Models" :
			...
			logistic_regression = Image.open("resources\Images\logistic_reg_perfomance.jpg")
			...
			"""
			svm = Image.open("resources\Images\support_vector1.jpg")
			...
			svm_perf = Image.open("resources\Images\support_vector_perfomance.jpg")
	...
	if selection == 'Prediction':
		...
		prediction_labels = {'Neutral':0,'Pro':1,'tweet':2,'Anti':3}
		if st.button("Classify"):
			st.text("Original test ::\n{}".format([tweet_text]))
			if model_choice == 'Logistic Regression':
				predictor = load_prediction_models("resources\lgm.pkl")
				prediction = predictor.predict([tweet_text])
				# st.write(prediction)
			elif model_choice == 'Random Forest Tree':
				predictor = load_prediction_models("resources\rfm.pkl")
				prediction = predictor.predict([tweet_text])
				# st.write(prediction)
	...
	if selection == 'NLP':
		...
		if st.checkbox("Wordcloud"):
			...
			wordcloud =  WordCloud(max_font_size=30, max_words=100, background_color="white").generate(tweet_text)
